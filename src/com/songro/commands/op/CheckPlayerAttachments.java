    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        try {
            if(player.isOp()) {
                assert target != null;
                if (args[1].length() == 0) {
                    player.sendMessage(ChatColor.RED + PluginCore.plugin.svname + " 플레이어 이름이 없습니다.");
                    player.sendMessage(ChatColor.RED + PluginCore.plugin.svname + " 명령어 사용방법: /<command> <Player>");
                } else {
