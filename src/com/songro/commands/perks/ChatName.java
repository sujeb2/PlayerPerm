            ...
            try {
                if(player.hasPermission("perks.plus") || player.hasPermission("def.op") || player.hasPermission("perks.pp") || player.hasPermission("perks.ultra")) {
                    if (msg.equals(player.getDisplayName())) {
                        player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 변경된게 없습니다.");
                        return true;
                    }
                    if (s.length() == 0) {
                        player.sendMessage(ChatColor.RED + PluginCore.plugin.svname + " 추가 명령어가 없습니다. 공백인경우 다른걸로 해주세요.");
                        player.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + " 명령어 사용방법: /<command> <바꿀 이름>");
                        return true;
                        ...
                        try {
                            ...
                            player.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + " 성공적으로 이름을 " + changedName + "으로 바꾸었습니다.");
                            return true;
                        } catch (Exception e) {
