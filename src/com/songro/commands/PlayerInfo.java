            ...
            try {
                ...
                if (args[0].equals("플레이어")) {
                    if (target == null) {
                        ...
                        player.sendMessage(ChatColor.DARK_GRAY + ": \uD83D\uDDE1 " + ChatColor.DARK_RED + "마지막으로 받은 데미지: " + target.getLastDamage());
                        if(pev == "오버월드") {
                            player.sendMessage(ChatColor.DARK_GRAY + ": " + ChatColor.WHITE  + "현재 '" + ChatColor.GREEN + pev + ChatColor.WHITE +"' 에 있는중");
                        } else if(pev == "네더") {
                            player.sendMessage(ChatColor.DARK_GRAY + ": " + ChatColor.WHITE  + "현재 '" + ChatColor.RED + pev + ChatColor.WHITE + "' 에 있는중");
                        } else if(pev == "엔드") {
                            player.sendMessage(ChatColor.DARK_GRAY + ": " + ChatColor.WHITE + "현재 '" + ChatColor.GRAY + pev + ChatColor.WHITE + "' 에 있는중");
                        }
                        player.sendMessage(String.format(ChatColor.DARK_GRAY + ": " + ChatColor.GRAY + "현재 위치: X: " + target.getLocation().getX() + " ,Y: " + target.getLocation().getY() + " ,Z: " + target.getLocation().getZ(), 2f));
                    }
                }
                return true;
            } catch (Exception e) {
