        ...
        try {
            try {
                ...
                if (AFK.toggleAFK(player)) {
                    ...
                    player.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + " 잠수가 풀렸습니다");
                ...
                player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 오류 코드: 0x41");
            }

        } catch (Exception e) {
            player.sendMessage(ChatColor.RED + PluginCore.plugin.svname +" 오류가 발생했습니다");
            player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 오류 로그: " + e);
            player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 오류 코드: 0x40");
        }
