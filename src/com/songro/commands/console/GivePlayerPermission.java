                ...
                if (player.hasPermission("def.op") || player.getDisplayName().equals("CONSOLE")) {
                    if (args[1].equalsIgnoreCase("plus")) {
                        ...
                        if (target.isPermissionSet("perks.p") || target.hasPermission("def.op")) {
                            player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 이미 " + target.getDisplayName() + "님은  " + ChatColor.WHITE + "'perks.plus'" + ChatColor.YELLOW + " 보다 높거나 같은 권한을 가지고 있습니다.");
                        } else {
                            player.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + target.getDisplayName() + "님에게 " + ChatColor.AQUA + "'perks.plus'" + ChatColor.GREEN + " 권한을 주었습니다!");
                            try {
                                if(target != null) {
                                    target.addAttachment(PluginCore.getPlugin(), "perks.pp", false);
                                    target.addAttachment(PluginCore.getPlugin(), "perks.ultra", false);
                                    target.addAttachment(PluginCore.getPlugin(), "perks.plus", true);
                                ...
                                player.sendMessage(ChatColor.YELLOW + "오류 로그: " + e + PluginCore.plugin.svname + "\n 오류 코드: 0x81");
                            }
                            if (target.isOnline()) {
                                target.playSound(target.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 1);
                                target.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + " 축하합니다! 이제 " + ChatColor.GREEN + "'Fruit+'" + ChatColor.GREEN + " 해택을 경험할수 있습니다!");
                            } else {
                                player.sendMessage(PluginCore.plugin.svname + " 해당 플레이어가 온라인이 아니여 메세지를 보내지 못했습니다.");
                            }
                        }
                    }
                    if (args[1].equalsIgnoreCase("plusplus")) {
                        ...
                        if (target.isPermissionSet("perks.pp") || target.hasPermission("def.op")) {
                            player.sendMessage(ChatColor.YELLOW + PluginCore.plugin.svname + " 이미 " + target.getDisplayName() + "님은 " + ChatColor.GOLD + "'perks.plusplus'" + ChatColor.YELLOW + " 권한을 가지고 있습니다.");
                        } else {
                            player.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname +  target.getDisplayName() + "님에게 " + ChatColor.GOLD + "'perks.plusplus'" + ChatColor.GREEN + " 권한을 주었습니다!");
                            try {
                                if(target != null) {
                                    target.addAttachment(PluginCore.getPlugin(), "perks.pp", true);
                                    target.addAttachment(PluginCore.getPlugin(), "perks.ultra", false);
                                    ...
                                    player.sendMessage(ChatColor.RED + PluginCore.plugin.svname + " 해당 플레이어는 현재 온라인이 아닙니다");
                                }
                            } catch (Exception e) {
                                log.severe("오류가 발생했습니다");
                                log.severe("오류 로그: " + e);
                            ...
                            if (target.isOnline()) {
                                target.playSound(target.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 1);
                                target.sendMessage(ChatColor.GREEN + PluginCore.plugin.svname + " 축하합니다! 이제 " + ChatColor.GOLD + "'Fruit++'" + ChatColor.GREEN + " 해택을 경험할수 있습니다!");
                            } else {
                    ...
                    if (args[1].equalsIgnoreCase("ultra")) {
                        ...
                        if (target.isPermissionSet("perks.ultra") || target.hasPermission("def.op")) {
                            ...
                            try {
                                if(target != null) {
                                    target.addAttachment(PluginCore.getPlugin(), "perks.pp", false);
                                    target.addAttachment(PluginCore.getPlugin(), "perks.ultra", true);
                                    target.addAttachment(PluginCore.getPlugin(), "perks.plus", false);
                    ...
                    if (args[1].equalsIgnoreCase("mutebypass")) {
                        ...
                        if (target.isPermissionSet("perks.mutebypass") || target.hasPermission("def.op")) {
                            ...
                            try {
                                if(target != null) {
                                    target.addAttachment(PluginCore.getPlugin(), "def.mutebypass", true);
                                    target.recalculatePermissions();
                                    player.sendMessage("1");
                                } else {
                                    player.sendMessage(ChatColor.RED + "[PlayerPerms] 해당 플레이어는 현재 온라인이 아닙니다");
